...
// 나이가 5~18세 범위 밖이면 가정형태와 무관하게 "아직 조금 더 커야 해요"/"이미 나이가
// 지났어요" 안내부터 보여줍니다 — 팀원분이 wpSubmitBirth에서 발견했던 "5세 미만인데도
...
function wpSubmitAll(){
  ...
  if(docBtn) docBtn.style.display = (household !== 'unsure' && ageEligible) ? 'flex' : 'none';
...
/* ============================================================================
   SPOO 기능 추가 ① — 알림 설정 (신청기간 · 결제마감 · 매달 이용)
   ============================================================================
   기존 renderApplyCountdown()의 "매번 앱을 열 때 날짜를 계산해 배너로 보여주는"
   방식을 그대로 따릅니다 — 서버·회원가입·백엔드 없이 100% 클라이언트에서 동작합니다.

   ⚠️ 중요한 기술적 한계 (팀 내부 공유용 — 사용자에게도 알림 설정 화면에 안내 문구로 노출):
   GitHub Pages 정적 호스팅에는 실제 서버 푸시(Web Push)를 보낼 서버가 없습니다.
   그래서 이 기능은 "앱을 열었을 때 그 시점 기준으로 알림을 보여주는" 방식입니다.
     1) 기본(항상 동작, 권한 불필요): 화면 상단 배너로 표시 — 기존 renderApplyCountdown과 동일
     2) 추가(브라우저 알림 권한 필요): 방문 시점에 Notification API로 알림을 "즉시" 띄웁니다.
        브라우저가 완전히 닫혀 있을 때 알아서 울리는 진짜 백그라운드 푸시는 아닙니다.
        (PWA를 설치한 안드로이드/크롬 환경에서는 상대적으로 더 잘 동작하고,
         iOS Safari는 백그라운드 알림 자체가 제한적입니다.)
   과장 없이 "이 기기에서, 앱을 열어볼 때 챙겨드려요" 정도로 안내하는 게 정직합니다.
============================================================================ */

const NOTIF_PREFS_KEY = 'spoo_notif_prefs';

function getNotifPrefs(){
  try {
    const saved = JSON.parse(localStorage.getItem(NOTIF_PREFS_KEY));
    return saved || { applyOn: true, applyDays: 7, payOn: true, monthlyOn: false };
  } catch(e){
    return { applyOn: true, applyDays: 7, payOn: true, monthlyOn: false };
  }
}
...
// index.html의 #notifSettingsOverlay / #notifSettingsContent 를 사용합니다 (region-view-overlay 패턴 재사용)
function openNotifSettings(){
  renderNotifSettingsUI();
  document.getElementById('notifSettingsOverlay')?.classList.add('show');
}
function closeNotifSettings(){
  document.getElementById('notifSettingsOverlay')?.classList.remove('show');
}
// 재방문자 홈(Return 메뉴)에서 바로 진입할 때 — wpGoToOperatorPage()와 동일한 패턴
function wpOpenNotifSettings(){
  document.getElementById('welcomeOverlay')?.classList.remove('show');
  goToStep(1);
  openNotifSettings();
}
...
function renderNotifSettingsUI(){
  ...
  const root = document.getElementById('notifSettingsContent');
  if(!root) return;

  const dayChip = (d) => `<button class="notif-chip ${prefs.applyDays===d?'active':''}" onclick="setApplyDays(${d})">D-${d}</button>`;

  root.innerHTML = `
    <div class="notif-card">
      <div class="notif-card-row">
        <div class="notif-card-icon notif-icon-blue">⏰</div>
        <div class="notif-card-text">
          <div class="notif-card-title" data-i18n="notif_apply_title">신청 기간 알림</div>
          <div class="notif-card-sub" data-i18n="notif_apply_sub">신청 시작 전에 미리 알려드려요</div>
        </div>
        <label class="notif-switch">
          <input type="checkbox" ${prefs.applyOn?'checked':''} onchange="toggleNotifPref('applyOn', this.checked)" aria-label="신청 기간 알림 켜기">
          <span></span>
        </label>
      </div>
      ${prefs.applyOn ? `<div class="notif-chip-row">${[7,3,1].map(dayChip).join('')}</div>` : ''}
    </div>

    <div class="notif-card">
      <div class="notif-card-row">
        <div class="notif-card-icon notif-icon-red">💳</div>
        <div class="notif-card-text">
          <div class="notif-card-title" data-i18n="notif_pay_title">결제 마감 알림</div>
          <div class="notif-card-sub" data-i18n="notif_pay_sub">12월 10일까지 결제하지 않으면 선정이 취소돼요</div>
        </div>
        <label class="notif-switch">
          <input type="checkbox" ${prefs.payOn?'checked':''} onchange="toggleNotifPref('payOn', this.checked)" aria-label="결제 마감 알림 켜기">
          <span></span>
        </label>
      </div>
    </div>
    ...
    <div class="notif-card">
      <div class="notif-card-row">
        <div class="notif-card-icon notif-icon-mint">📅</div>
        <div class="notif-card-text">
          <div class="notif-card-title" data-i18n="notif_monthly_title">매달 이용 알림</div>
          <div class="notif-card-sub" data-i18n="notif_monthly_sub">3개월 연속 미사용 시 지원이 중단돼요</div>
        </div>
        <label class="notif-switch">
          <input type="checkbox" ${prefs.monthlyOn?'checked':''} onchange="toggleNotifPref('monthlyOn', this.checked)" aria-label="매달 이용 알림 켜기">
          <span></span>
        </label>
      </div>
    </div>

    <div class="notif-note" data-i18n="notif_note">
      🔔 홈 화면 알림을 받으려면 브라우저 알림을 허용해주세요. 이 기기에서만 동작하고 서버로 아무 정보도 전송하지 않아요.
      다만 앱이 완전히 꺼져 있을 땐 못 받을 수 있어요 — 가끔 SPOO를 열어봐 주시면 그때 바로 알려드려요.
    </div>
  `;
}
...
function toggleNotifPref(key, val){
  ...
  setNotifPrefs(prefs);
  renderNotifSettingsUI();
  if(val) maybeRequestNotifPermission();
}
function setApplyDays(d){
  const prefs = getNotifPrefs();
  prefs.applyDays = d;
  setNotifPrefs(prefs);
  renderNotifSettingsUI();
...
// 실제 알림 발송 — 같은 tag로 하루 한 번만 (중복 방지)
function fireLocalNotification(title, body, tag){
  if(!('Notification' in window) || Notification.permission !== 'granted') return;
  const sentKey = `spoo_notif_sent_${tag}`;
  const todayStr = localDateStr();
  if(localStorage.getItem(sentKey) === todayStr) return;
  try {
    ...
    localStorage.setItem(sentKey, todayStr);
  } catch(e){ /* 권한 없음·미지원 브라우저는 조용히 무시 */ }
}

// loadConfig() 안에서 renderApplyCountdown()과 나란히 호출해주세요 (아래 삽입 가이드 참고)
function checkDeadlineNotifications(){
  ...
  if(prefs.applyOn && today < start){
    const dUntil = Math.round((start - today) / MS_DAY);
    if(dUntil === prefs.applyDays){
      const dateLabel = spooConfig.applyPeriod.start.slice(5).replace('-', '월 ') + '일';
      fireLocalNotification(
        'SPOO',
        t('notif_fire_apply', `신청 기간이 ${prefs.applyDays}일 남았어요 — ${dateLabel}부터 신청할 수 있어요`)
      , `apply_${prefs.applyDays}`);
    }
  }

  if(prefs.payOn && spooConfig.paymentDeadline){
    const pay = new Date(spooConfig.paymentDeadline + 'T00:00:00');
    const dUntilPay = Math.round((pay - today) / MS_DAY);
    if(dUntilPay === 3 || dUntilPay === 0){
      fireLocalNotification(
        'SPOO',
        t('notif_fire_pay', '결제 마감이 얼마 남지 않았어요 — 본인부담금 결제를 확인해주세요')
      , `pay_${dUntilPay}`);
    }
  }

  if(prefs.monthlyOn && today.getDate() === 25){
    fireLocalNotification(
      'SPOO',
      t('notif_fire_monthly', '이번 달 아직 결제 전이면 확인해보세요 — 3개월 연속 미사용 시 지원이 중단돼요')
    , `monthly_${todayStr.slice(0,7)}`);
  }
}


/* ============================================================================
   SPOO 기능 추가 ② — 이용 진행 체크리스트
   ============================================================================
   자가진단 → 서류준비 → 신청 → 카드발급/첫결제, 4단계 진행 상황을 한 화면에 보여줍니다.
   1·2단계는 새로 저장하지 않고 이미 있는 값을 그대로 읽습니다:
     - 1단계(자가진단 완료) = localStorage 'fairplay_onboarded'
     - 2단계(서류 준비)     = 기존 doc-checklist의 'fp_doc_{household}_{i}' 체크 상태
   3·4단계(신청/결제)는 앱이 확인할 방법이 없어 사용자가 직접 체크하는 자기 보고 방식입니다.
============================================================================ */

function getProgressSteps(){
  const onboarded = localStorage.getItem('fairplay_onboarded') === '1';
  const household = localStorage.getItem('fairplay_household');

  let docsTotal = 0, docsDone = 0;
  if(household){
    const prefix = `fp_doc_${household}_`;
    const keys = Object.keys(localStorage).filter((k) => k.startsWith(prefix));
    docsTotal = keys.length;
    docsDone = keys.filter((k) => localStorage.getItem(k) === '1').length;
  }

  const applied = localStorage.getItem('spoo_step_applied') === '1';
  const paid = localStorage.getItem('spoo_step_paid') === '1';

  return [
    {
      id: 'diagnose',
      label: t('step_diagnose', '자가진단 완료'),
      sub: t('step_diagnose_sub', '대상 여부와 우선순위를 확인했어요'),
      done: onboarded,
      manual: false
    },
    {
      id: 'docs',
      label: t('step_docs', '필요 서류 준비'),
      sub: docsTotal
        ? `${docsDone}/${docsTotal} ${t('step_docs_sub_progress', '체크 완료')}`
        : t('step_docs_sub_empty', '자가진단 결과에서 서류를 먼저 확인해주세요'),
      done: docsTotal > 0 && docsDone === docsTotal,
      manual: false
    },
    {
      id: 'applied',
      label: t('step_applied', '온라인 신청하기'),
      sub: t('step_applied_sub', 'svoucher.kspo.or.kr에서 신청서를 제출해요'),
      done: applied,
      manual: true
    },
    {
      id: 'paid',
      label: t('step_paid', '카드 발급 & 첫 결제'),
      sub: t('step_paid_sub', '신한카드 발급 후 강좌를 결제하면 끝이에요'),
      done: paid,
      manual: true
    }
  ];
...
function toggleProgressStep(id){
  const key = id === 'applied' ? 'spoo_step_applied' : 'spoo_step_paid';
  const cur = localStorage.getItem(key) === '1';
  localStorage.setItem(key, cur ? '0' : '1');
  renderProgressChecklist();
}

function openProgressChecklist(){
  renderProgressChecklist();
  document.getElementById('progressViewOverlay')?.classList.add('show');
}
function closeProgressChecklist(){
  document.getElementById('progressViewOverlay')?.classList.remove('show');
}
function wpOpenProgress(){
  document.getElementById('welcomeOverlay')?.classList.remove('show');
  goToStep(1);
  openProgressChecklist();
}
...
function renderProgressChecklist(){
  ...
  const root = document.getElementById('progressViewContent');
  if(!root) return;

  const pct = Math.round((doneCount / steps.length) * 100);
  const summaryTitle = doneCount >= steps.length
    ? t('progress_all_done', '다 왔어요! 🎉')
    : t('progress_left_n', '{n}단계 남았어요').replace('{n}', steps.length - doneCount);

  const stepsHtml = steps.map((s, i) => {
    const current = i === firstUndoneIdx;
    const dotAttrs = s.manual
      ? `onclick="toggleProgressStep('${s.id}')"`
      : `disabled style="cursor:default;"`;
    return `
      <div class="progress-step ${current ? 'current' : ''} ${s.done ? 'done' : ''}">
        <button class="progress-dot" ${dotAttrs} aria-label="${escapeAttr(s.label)} 완료 표시">
          ${s.done ? '✓' : ''}
        </button>
        <div class="progress-step-text">
          <div class="progress-step-title">
            ${escapeHtml(s.label)}
            ${current ? `<span class="progress-badge">${t('step_next', '다음 단계')}</span>` : ''}
          </div>
          <div class="progress-step-sub">${escapeHtml(s.sub)}</div>
        </div>
      </div>`;
  }).join('');
  ...
  root.innerHTML = `
    <div class="progress-summary">
      <div class="progress-summary-top">
        <div class="progress-summary-title">${summaryTitle}</div>
        <div class="progress-summary-count">${doneCount}/${steps.length} ${t('progress_done_suffix', '완료')}</div>
      </div>
...
/* ============================================================================
   checkDeadlineNotifications() 자동 실행 — 검색으로 못 찾아도 괜찮아요
   ============================================================================
   원래는 renderConfigNotices(); 줄 바로 아래에 checkDeadlineNotifications();
   한 줄을 추가하는 방법을 안내드렸는데, 이 코드는 그걸 대신해줍니다.
   spooConfig(신청기간·결제마감 정보)가 다 불러와질 때까지 0.5초마다 확인하다가,
   준비되면 자동으로 checkDeadlineNotifications()를 실행해요.
   (최대 10초까지만 시도하고, 그래도 안 되면 조용히 포기해요 — 에러 안 납니다)
============================================================================ */
(function(){
  let tries = 0;
  const iv = setInterval(() => {
    tries++;
    if (typeof spooConfig !== 'undefined' && spooConfig) {
